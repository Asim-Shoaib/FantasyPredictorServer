...
import { Shield, Zap, Scale, Star } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{
          background: franchise.primary + '25',
          color: franchise.secondary,
          border: `1px solid ${franchise.primary}40`,
        }}
      >
        {franchise.shortName}
      </span>
...
function ScoreRangeBar({ floor, expected, ceiling }: { floor: number; expected: number; ceiling: number }) {
  const range = ceiling - floor || 1
  const expectedPct = Math.min(Math.max(((expected - floor) / range) * 100, 0), 100)

  return (
    <div className="px-4 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      {/* Track */}
      <div className="relative h-5 flex items-center">
        <div className="absolute inset-x-0 h-[2px] rounded-full" style={{ background: 'rgba(255,255,255,0.08)' }} />
        {/* Filled floor → expected */}
        <div
          className="absolute h-[2px] rounded-full"
          style={{
            left: 0,
            width: `${expectedPct}%`,
            background: 'linear-gradient(90deg, rgba(239,68,68,0.5), #00c896)',
          }}
        />
        {/* Floor dot */}
        <div
          className="absolute w-2 h-2 rounded-full"
          style={{ left: 0, transform: 'translateX(-50%)', background: 'rgba(239,68,68,0.7)', border: '1.5px solid rgba(239,68,68,0.4)' }}
        />
        {/* Expected dot */}
        <div
          className="absolute w-2.5 h-2.5 rounded-full"
          style={{ left: `${expectedPct}%`, transform: 'translateX(-50%)', background: '#00c896', boxShadow: '0 0 6px #00c89680', border: '1.5px solid #00c896' }}
        />
        {/* Ceiling dot */}
        <div
          className="absolute w-2 h-2 rounded-full"
          style={{ right: 0, transform: 'translateX(50%)', background: '#f59e0b', border: '1.5px solid rgba(245,158,11,0.5)' }}
        />
      </div>
      {/* Labels */}
      <div className="flex justify-between mt-1.5">
        <div className="text-center">
          ...
          <p className="text-xs font-black" style={{ color: 'rgba(239,68,68,0.8)' }}>{fmt(floor, 0)}</p>
        </div>
        <div className="text-center">
          <p className="text-[10px] text-slate-500">Expected</p>
          <p className="text-xs font-black" style={{ color: '#00c896' }}>{fmt(expected, 0)}</p>
        </div>
        <div className="text-center">
          <p className="text-[10px] text-slate-500">Ceiling</p>
          <p className="text-xs font-black" style={{ color: '#f59e0b' }}>{fmt(ceiling, 0)}</p>
        </div>
      </div>
    </div>
  )
